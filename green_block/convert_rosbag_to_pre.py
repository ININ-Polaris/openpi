    ...
    with AnyReader([ep_path]) as reader:
        ...
        for connection, t_ns, raw in reader.messages(connections=connections):
            ...
            if topic in images_topics and msg_type == "sensor_msgs/msg/CompressedImage":
                ...
                if topic == "/right/teleop/hand_val":
                    trh_pairs.append((t_ns * NS2S, val))
                    # TODO
                    tlh_pairs.append((t_ns * NS2S, val))
                elif topic == "/left/teleop/hand_val":
                    tlh_pairs.append((t_ns * NS2S, val))
                else:
                    raise RuntimeError("Error Hand")
...
def main(
    ...
    rosbag_files = sorted(raw_dir.glob("episode_*"))
...
# uv run green_block/convert_rosbag_to_pre.py --raw_dir /home/charles/tmp/green_block --out_root runtime/pre_green --prompt 'Put the green block on the cloth.'
